...
func SignRequestWithDigest(r *http.Request, url *url.URL, keyID string, privateKey *rsa.PrivateKey, body []byte) (*http.Request, error) {
	...
	newSignature := strings.Replace(signature, "algorithm=\"hs2019\"", "algorithm=\"rsa-sha256\"", 1)
	log.Infof("Changed Signature=%q to Signature=%q", signature, newSignature)
	r.Header.Set("Signature", newSignature)
	return r, nil
}

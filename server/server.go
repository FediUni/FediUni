...
func (s *Server) getActorInbox(w http.ResponseWriter, r *http.Request) {
	...
	notifications := strings.ToLower(r.URL.Query().Get("notifications")) == "true"
	...
	if !page {
		...
		case notifications:
			log.Infof("Getting Notifications from Inbox as OrderedCollection")
			inbox, err = s.Actor.GetNotificationsInboxAsOrderedCollection(ctx, username)
			if err != nil {
				log.Errorf("Failed to read from Notifications Inbox of Username=%q: got err=%v", username, err)
				http.Error(w, fmt.Sprintf("failed to load actor inbox"), http.StatusInternalServerError)
				return
			}
		default:
		...
		case notifications:
			log.Infof("Getting Notifications from Inbox as OrderedCollectionPage")
			inbox, err = s.Actor.GetNotificationsInboxPage(ctx, username, minID, maxID)
			if err != nil {
				log.Errorf("Failed to read from Notifications Inbox of Username=%q: got err=%v", username, err)
				http.Error(w, fmt.Sprintf("failed to load actor inbox"), http.StatusInternalServerError)
				return
